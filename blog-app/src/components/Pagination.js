            ...
            <div className="p-2 flex flex-wrap">
                {pagesArray.map((page) => (
                    <span onClick={() => updateCurrentPageIndex(page)} key={page}
                        className={`${activePageIndex === page ? "bg-green-600 hover:bg-green-400 text-white border my-2 border-gray-900 p-2 mx-2 rounded-md" : "border my-2 border-green-400 p-2 mx-2 rounded-md"} `}>
                        {page}
                    </span>
                ))}
            </div>
