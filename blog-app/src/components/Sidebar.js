...
export class Sidebar extends Component {
  ...
  render() {
    ...
    return (
      <aside className="bg-slate-100 rounded-md p-4 shadow-lg">
        <h3 className="text-2xl py-4 text-center font-bold"> Popular Tags </h3>
        <h4 className="flex flex-wrap">
          {tags.map((tag) => (
            <span
              key={tag}
              onClick={() => this.props.addTab(tag)}
              className="bg-slate-600 hover:bg-slate-500 p-2 mx-2 my-2 rounded-md text-slate-50"
            >
              {tag}
            </span>
          ))}
